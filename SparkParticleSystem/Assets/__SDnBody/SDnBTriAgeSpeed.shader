Shader "SDnBshades/TriPartAgeSpeedShader" 
{
	...
	SubShader 
	{
		...
		v2g vert(uint id : SV_VertexID)
		{
			...
			float ageabove =  max(_VertDat[relID].y,0);
			OUT.col = float4( frac(ageabove* _Color.r ),frac(ageabove*_Color.g) , frac(ageabove*_Color.b),1);
			OUT.col = OUT.col*0.55 + (ageabove*0.1)*0.45;
			OUT.col.a = _Color.a;

			OUT.col = lerp(OUT.col,_DeadColor, saturate( -sign(_VertDat[relID].y) ));
		...
		{
			//return IN.colo; 
			return float4(IN.colo.rgb ,1)*_Color.a; 
		}
